...
class WattPad:
	...
	def get_password_strength_policies(self):
		return requests.get(
			f"{self.first_api}/5/password-strength/policies",
			headers=self.headers).json()

	def check_password_strength(self, username: str, password: str):
		data = {
			"password": password,
			"requester": {
				"username": username
	...
	def get_user_wallet(self, username: str):
		return requests.get(
	...
	def get_user_subscription_prompts(self, username: str):
		return requests.get(
	...
	def ignore_user(self, username: str):
		data = {
			"id": username,
			"action": "ignore_user"
		}
	...
	def unignore_user(self, username: str):
		data = {
			"id": username,
	...
	def delete_comment(self, username: str, message_id: int):
	...
	def get_comment_replies(
			self,
			message_id: int,
			fields: str = "replies(id,body,createDate,from),nextUrl"):
		return requests.get(
	...
	def search_stories(
			...
			paid: int = 1,
			limit: int = 30,
			fields: str = "stories(id,title,voteCount,readCount,numParts,tags,description,user,mature,completed,rating,cover,promoted,isPaywalled,lastPublishedPart,sponsor(name,avatar),tracking(clickUrl,impressionUrl,thirdParty(impressionUrls,clickUrls)),contest(endDate,ctaLabel)),tags,nextUrl,total"):
		return requests.get(
	...
	def get_started_chats(self, limit: int = 20):
		return requests.get(
	...
	def update_fullname(self, fullname: str):
		data = {"name": fullname}
		return requests.put(
	...
	def update_website(self, url: str):
		data = {"website": url}
		return requests.put(
			f"{self.first_api}/api/v3/users/{self.username}?wp_token={self.wp_token}",
			data=data,
